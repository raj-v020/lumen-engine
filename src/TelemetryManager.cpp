...
TelemetryManager::~TelemetryManager() { shutdown(); }
...
void TelemetryManager::capture_trace(
    std::unique_ptr<core::InferenceTrace> trace) {
  {
  ...
  while (m_running) {
...

    {
      ...
      m_cv.wait_for(lock, std::chrono::seconds(1),
                    [this] { return !m_trace_buffer.empty() || !m_running; });

      if (!m_running && m_trace_buffer.empty())
        break;
...
void TelemetryManager::process_snapshot(
    std::vector<std::unique_ptr<core::InferenceTrace>> &batch) {
  size_t count = batch.size();
  if (count == 0)
    return;

  double sum_e2e = 0.0, sum_queue = 0.0, sum_pre = 0.0, sum_infer = 0.0,
         sum_post = 0.0;
  ...
  if (p99_idx >= count)
    p99_idx = count - 1;
  std::nth_element(latencies.begin(), latencies.begin() + p99_idx,
                   latencies.end());
  double p99_val = latencies[p99_idx];

  std::cout << "\033[1;32m[LUMEN TELEMETRY]\033[0m "
            << "Batch: " << count << " reqs | "
            << "Avg E2E: " << std::fixed << std::setprecision(2)
            << (sum_e2e / count) << "ms | "
            << "P99: " << p99_val << "ms\n"
            << "  └─ Breakdown: [Queue: " << (sum_queue / count)
            << "ms, Pre: " << (sum_pre / count)
            << "ms, Infer: " << (sum_infer / count)
            << "ms, Post: " << (sum_post / count) << "ms]" << std::endl;
}

void TelemetryManager::save_to_csv(const core::InferenceTrace &trace,
  ...
  auto &config = lumen::utils::ConfigManager::get();
  std::string q_name;
  switch (config.get_queue_type()) {
  case utils::QueueType::SPSC:
    q_name = "SPSC";
    break;
  case utils::QueueType::MPMC:
    q_name = "MPMC";
    break;
  case utils::QueueType::BATCHED:
    q_name = "Batched";
    break;
  default:
  case utils::QueueType::NAIVE_MUTEX:
    q_name = "Naive";
    break;
  }

  std::string a_name =
      (config.get_alloc_type() == utils::AllocatorType::LUMEN_ARENA)
          ? "Arena"
          : "Standard";

  if (!exists || std::filesystem::file_size(path) == 0) {
    file << "timestamp,request_id,queue_type,alloc_type,queue_ms,preprocess_ms,"
            "inference_ms,postprocess_ms,total_ms\n";
  }

  double total = trace.queue_wait_ms + trace.preprocess_ms +
                 trace.inference_ms + trace.postprocess_ms;

  file << std::chrono::system_clock::to_time_t(std::chrono::system_clock::now())
       << "," << trace.request_id << "," << q_name << "," << a_name << ","
       << std::fixed << std::setprecision(4) << trace.queue_wait_ms << ","
       << trace.preprocess_ms << "," << trace.inference_ms << ","
       << trace.postprocess_ms << "," << total << "\n";
}
