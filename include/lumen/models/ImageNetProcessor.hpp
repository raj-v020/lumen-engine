...
public:
  void transform(const uint8_t *pixels, float *tensor_buffer, size_t width,
...
public:
  explicit ImageNetPostProcessor(const std::string &label_path) {
    ...
    if (file.is_open()) {
      ...
      std::cerr << "[Lumen Warning] Labels file not found: " << label_path
                << std::endl;
    }
  }
  ...
  std::string handle_results(const float *results_ptr, size_t count) override {
    if (!results_ptr || count == 0)
      return "Error: Empty or Invalid Tensor";
    ...
    std::string label_name =
        (class_id < (int)labels.size()) ? labels[class_id] : "Unknown";
